...
class CoreListener extends Listener_1.Listener {
    ...
    async run(payload) {
        ...
        if (!globalResult.success) {
            message.client.emit(Events_1.Events.MessageCommandDenied, globalResult.error, payload);
            return;
        }
        // Run command-specific preconditions:
        const localResult = await command.preconditions.messageRun(message, command, payload);
        if (!localResult.success) {
            message.client.emit(Events_1.Events.MessageCommandDenied, localResult.error, payload);
            return;
        }
